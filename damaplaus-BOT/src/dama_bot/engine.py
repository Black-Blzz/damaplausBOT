...
def legal_moves(position: Position) -> list[Move]:
    """Brazilian-draughts moves, including mandatory *maximum* capture rule."""
    board = position.mapping()
    captures: list[Move] = []
    for square, piece in board.items():
        if piece.color is position.turn:
            # The moving piece vacates its origin for the entire capture search.
            # Captured pieces remain as blockers until turn end (Brazilian rule).
    ...
    if captures:
        maximum = max(len(m.captures) for m in captures)
        return [m for m in captures if len(m.captures) == maximum]
...
def _quiet_moves(board: dict[Square, Piece], start: Square, piece: Piece) -> list[Move]:
    ...
    for direction in directions:
        ...
        while inside(target) and target not in board:
            ...
            if not piece.king:
                break
            target = advance(target, direction)
    return moves
...
def _capture_moves(board: dict[Square, Piece], current: Square, piece: Piece,
    ...
    """Captured men block traversal until turn end; never capture one twice."""
    options: list[tuple[Square, Square]] = []
    for direction in DIRS:  # Men capture both forward and backward.
        cursor = advance(current, direction)
        if piece.king:
            while inside(cursor) and (cursor not in board or cursor in previously_captured):
                cursor = advance(cursor, direction)
            if not inside(cursor):
                continue
        if not piece.king and not inside(cursor):
            continue
        victim = cursor
        victim_piece = board.get(victim)
        if not victim_piece or victim_piece.color is piece.color or victim in previously_captured:
            continue
        landing = advance(victim, direction)
        while inside(landing) and (landing not in board or landing in previously_captured):
            options.append((victim, landing))
            if not piece.king:
                break
            landing = advance(landing, direction)
    if not options:
...
class StrongEngine:
    ...
    @staticmethod
    def _beginner(moves: list[Move]) -> Move:
        """A roughly 500-strength opponent: it knows the rules, not strategy."""
        # legal_moves already enforces mandatory/maximum captures.  Choosing at
        # random from those moves deliberately leaves normal beginner mistakes
        # in place, instead of evaluating tactics or protecting material.
        return random.choice(moves)

    def _easy(self, position: Position, moves: list[Move]) -> Move:
        # Capture priority is already enforced. Avoid immediate material losses if possible.
        scored = [(self._one_ply_safety(position, move), move) for move in moves]
        best = max(score for score, _ in scored)
        return random.choice([move for score, move in scored if score >= best - 30])
